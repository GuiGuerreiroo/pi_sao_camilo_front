        ...
        <div className="px-6 pt-8 pb-4 max-w-lg mx-auto">
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col items-center justify-center w-full">
              <h2 className="text-xs font-semibold text-gray-500 mb-3">Tempo de Sessão</h2>

              {/* Content Area */}
              <div className="w-full max-w-[320px]">
                {!showManualTime ? (
                  // Cronômetro View (Large Gray Box)
                  <div className="flex flex-col items-center justify-center bg-gray-200 rounded-[24px] pt-12 pb-10 px-6 h-[240px]">
                    <div className="text-5xl font-bold text-gray-900 mb-8 tracking-wider">
                      {formatTime(timer)}
                    </div>

                    <div className="flex items-center w-full max-w-[220px] h-12 shadow-md rounded-full bg-white">
                      <button
                      ...
                      <button
                        ...
                        className="flex-1 h-full flex items-center justify-center text-white bg-red-700 border-2 border-red-700 rounded-r-full active:bg-red-800 transition-colors pointer-events-auto"
                  ...
                  <div className="flex flex-col items-center w-full">
                    <div className="w-full">
                      <Input
                        ...
                        value={timer > 0 ? Math.floor(timer / 60).toString() : ""}
                        onChange={(e) => {
                          const val = e.target.value;
                          setTimer(val === "" ? 0 : Number(val) * 60);
                        }}
                        className={`text-center font-bold text-2xl h-[60px] bg-gray-200 text-gray-900 border-none rounded-[20px] shadow-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none placeholder-gray-400 ${isActive ? "opacity-50 cursor-not-allowed" : ""
                          }`}
                        placeholder="00:00"
                        suffix={<span className="text-sm font-bold text-gray-400 mr-4">min</span>}
                      />
                    </div>
                    {isActive && (
                      <p className="mt-4 text-sm text-red-500 text-center font-medium leading-tight">
                        Pause o cronômetro para ajustar manualmente.
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="relative flex w-full max-w-[320px] bg-gray-200 rounded-3xl p-0.5 mt-6 h-[50px]">
                <div
                  className={`absolute top-0.5 bottom-0.5 w-[calc(50%-2px)] bg-white shadow-sm rounded-[22px] transition-transform duration-300 ease-in-out ${showManualTime ? "translate-x-[calc(100%+2px)]" : "translate-x-0"
                    }`}
                />
                <button
                  onClick={() => setShowManualTime(false)}
                  className={`flex-1 relative z-10 flex items-center justify-center text-sm font-semibold transition-colors duration-300 outline-none ${!showManualTime ? "text-gray-900" : "text-gray-500 hover:text-gray-700"
                    }`}
                >
                  Cronômetro
                </button>
              ...
              {showUrineVolume && (
                <div className="animate-fade-in mt-2">
                  <Input
                    ...
                    placeholder="0"
                    error={error}
                    suffix={<span className="text-sm font-semibold text-gray-500">ml</span>}
                  />
                </div>
              )}
            </div>
